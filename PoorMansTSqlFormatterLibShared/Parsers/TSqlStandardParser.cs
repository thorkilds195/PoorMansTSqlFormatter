...
namespace PoorMansTSqlFormatterLib.Parsers
{
    public class TSqlStandardParser : ISqlTokenParser
    {
        ...
        // Since we generally are not so strict on drop identifier keyword (like table, view, procedure etc) in remaining of program, allow any string to be interpreted here
        static Regex _DropIfExistsDetector = new Regex(@"^DROP\s+\S+\s+IF\s+EXISTS\s+\S+");
            ...
            while (tokenID < tokenCount)
            {
                ...
                switch (token.Type)
                {
                    ...
                    case SqlTokenType.OtherNode:
                        ...
                        else if (_DropIfExistsDetector.IsMatch(significantTokensString))
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            ProcessCompoundKeyword(tokenList, sqlTree, sqlTree.CurrentContainer, ref tokenID, significantTokenPositions, 4);
                        }
